# Template Manager Animation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a looping SVG animation component for the Template Manager project card in the portfolio carousel, communicating the copy-to-library workflow.

**Architecture:** Pure inline SVG + `motion/react` animated props — identical pattern to `DonorDirectoryAnimation.tsx`. No new dependencies. `ProjectCarousel.tsx` adds one condition to render it in place of the static image for `gravyty-template-manager`.

**Tech Stack:** React, `motion/react` (import from `motion/react`), inline SVG
...
| Action | File |
|--------|------|
| Create | `src/components/showcase/TemplateManagerAnimation.tsx` |
| Modify | `src/components/showcase/ProjectCarousel.tsx` |

No tests — this is a pure presentational SVG loop with no extractable logic. Verification is visual.

---

### Task 1: Create TemplateManagerAnimation component

**Files:**
- Create: `src/components/showcase/TemplateManagerAnimation.tsx`

**Layout constants (baked into the SVG):**

```
ViewBox: 900 × 560         Accent: #7B8CDE    Background: #0f0f1a
Top nav:   y 0–50          (full width, #16162a)
Sidebar:   x 0–188         y 51–560  (#0d0d1e)
Main:      x 189–900       y 51–560

Sidebar items:
  Templates (active)  y 62–88
  Settings            y 96–122
  Users               y 126–152
  — divider y 158 —
  LIBRARIES label     y 165
  Annual Fund folder  y 173–203   ← animated highlight
  Success pill        y 207–227   ← animated
  Year-End folder     y 233–259
  Events folder       y 265–291
  Stewardship folder  y 297–323

Main items:
  Toolbar             y 51–94
  Table header        y 95–130
  Row 1               y 131–178
  Row 2               y 179–226
  Row 3 (ACTIVE)      y 227–274   ← animated highlight + accent bar
  Row 4               y 275–322
  Row 5               y 323–370

Action dropdown (floats over rows 2–3):  x 711, y 207, w 164, h 90
Ghost card:  starts x 246 y 233, moves Δx=-242 Δy=-61 toward Annual Fund folder
```

**Animation timing (8 s loop):**

| Element | Appears | Fades |
|---------|---------|-------|
| Row highlight + bar | t=0.125 (1 s) | t=0.875 (7 s) |
| Cursor pulse | fires at t=0.265 | |
| Action dropdown | t=0.250 (2 s) | t=0.875 (7 s) |
| Folder highlight | t=0.375 (3 s) | t=0.875 (7 s) |
| Ghost template | t=0.500 (4 s) | t=0.625 (5 s) |
| Success pill | t=0.625 (5 s) | t=0.9375 (7.5 s) |

- [ ] **Step 1: Create the file**

Create `src/components/showcase/TemplateManagerAnimation.tsx` with the full component:
...
const TemplateManagerAnimation: React.FC<Props> = ({ className }) => (
  ...
  >
    ...
    {/* ── Template ghost ── */}
    {/*
      Ghost card children are at x=246, y=233, w=180, h=32.
      Ghost card center: x=336, y=249.
      Annual Fund folder center: x=94, y=188.
      Delta to animate: x=-242, y=-61.
    */}
    <motion.g
      animate={{
        x: [0, 0, 0, -242, -242, 0],
...
```bash
git add src/components/showcase/TemplateManagerAnimation.tsx
git commit -m "feat: add TemplateManagerAnimation looping SVG component"
```

---

### Task 2: Wire into ProjectCarousel

**Files:**
- Modify: `src/components/showcase/ProjectCarousel.tsx`

The current render block (around line 142–149) is:
...
At the top of `src/components/showcase/ProjectCarousel.tsx`, after the `DonorDirectoryAnimation` import:
...
```bash
git add src/components/showcase/ProjectCarousel.tsx
git commit -m "feat: render TemplateManagerAnimation for template-manager card"
```
...
- [ ] **Step 2: Navigate to `/projects` and select the Code tab**

The Template Manager card should show the animated SVG (not the static `.svg` image).

- [ ] **Step 3: Verify the 8-second loop**

Watch one full cycle and confirm:

| Time | Expected |
|------|----------|
| 0–1s | Idle — list visible, no highlights |
| ~1s | Row 3 gets accent-blue left bar + tinted background |
| ~2s | Dropdown appears (Copy / Move / Set Default) |
| ~3s | Annual Fund folder highlights with left bar |
| ~4s | Ghost card appears on row 3 |
| ~5s | Ghost has flown to folder and faded; "Copied to Annual Fund" pill visible |
| ~6–7s | Success pill holds |
| ~7–8s | All animated elements fade, resets |

- [ ] **Step 4: Check overflow**

The ghost must not visually overflow the card. The `article` wrapper in `ProjectCarousel` has `overflow-hidden` — confirm the ghost clips cleanly at all four edges.

- [ ] **Step 5: Check no console errors**

Open browser DevTools. Confirm zero React/motion warnings.
