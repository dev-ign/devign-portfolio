...
const qaPath = path.resolve('artifacts/phase8b/qa/webkit-qa.json');
const screenshotPath = path.resolve('artifacts/phase8b/screenshots/webkit-desktop-1440x900.png');
...
const inspect = async (reducedMotion, viewport) => {
  ...
  const hiddenAtEnd = await page.evaluate(() =>
    Array.from(document.querySelectorAll('[data-reveal]')).filter((element) => {
...
const qa = {
  ...
  mobile: await inspect('no-preference', { width: 390, height: 844 }),
