...
interface SendInquiryEmailsOptions {
  resend: ResendLike;
  fromEmail: string;
  ownerEmail: string;
...
const formatProjectTypes = (projectTypes: ProjectTypeId[]) =>
...
export const generateLeadNotificationEmail = (
  inquiry: InquiryEmailData,
  assets: UploadedAsset[],
...
export const generateAutoReplyEmail = (inquiry: InquiryEmailData): EmailContent => {
...
const assertResendAccepted = (result: unknown) => {
  if (!isRecord(result) || !('error' in result)) return;
...
export const sendInquiryEmails = async ({
  ...
  assets,
  submittedAt,
}: SendInquiryEmailsOptions) => {
  const leadEmail = generateLeadNotificationEmail(inquiry, assets, submittedAt);
  ...
  try {
    const autoReplyEmail = generateAutoReplyEmail(inquiry);
